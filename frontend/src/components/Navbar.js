...
const Navbar = () => {
    //here , we are finding the state form the userReducer which we mentioned in Redux/userReducer.js
    const userData = useSelector(state => state.userReducer);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    useEffect(() => { // getuser from backend 
        const isLoggedIn = async () => {
            try {
                const { data } = await axios.get('/getuser');
                //if data is present ie. user
                if (data.user) {
    ...
    return (
        ...
        <div className={Styles['navbar']}>
            ...
            {userData.isLoggedIn && <NavLink className={Styles['nav-item']} to="/cart">
                <span className='cart-heading'>Cart
                    <span className='cart-number'>{userData?.cart?.length}</span>
                </span>
            </NavLink>}
